...
class TrajectoryModel(torch.nn.Module):
    def __init__(self, K, n_genes, n_conditions=2, beta_l1_strength=1.):
        super().__init__()
        self.K = K
    ...
    def forward(self, times, data):
        ...
        betas = pyro.sample(
            "beta",
            dist.Exponential(self.beta_l1_strength).expand([self.n_conditions, self.n_genes, self.K]).to_event(3),
        )
